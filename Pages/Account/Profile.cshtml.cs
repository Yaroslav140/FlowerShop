...
{
    public class ProfileModel(FlowerDbContext context, IEmailService emailService) : PageModel
    {
        ...
        public string? EditSuccessMessage { get; set; }
        ...
        public async Task<IActionResult> OnPostUpdateProfileAsync()
        {
            ...
            ModelState.Remove("EditInput.NewPhone");
            ModelState.Remove("NewPhone");
            ...
            var newEmail = EditInput.NewLogin.Trim().ToLowerInvariant();
            var emailChanged = !string.Equals(user.Email, newEmail, StringComparison.OrdinalIgnoreCase);

            if (emailChanged)
            {
                var emailExists = await _context.UserDomains
                    .AnyAsync(l => (l.Email == newEmail || l.PendingEmail == newEmail) && l.Id != userId);

                if (emailExists)
                {
                    ModelState.AddModelError(string.Empty, "Этот email уже используется другим пользователем.");
                    await LoadUserDataAsync(userId);
                    IsEditing = true;
                    return Page();
                }
                ...
                user.Phone = EditInput.NewPhone;
                await _context.SaveChangesAsync();

                var link = Url.PageLink("/Account/ConfirmEmailChange", values: new { token })!;
                try { await _emailService.SendEmailChangeConfirmationAsync(newEmail, user.Name, link); }
                catch { }

                await LoadUserDataAsync(userId);
                EditSuccessMessage = $"Письмо с подтверждением отправлено на {newEmail}. Email изменится после подтверждения.";
                return Page();
            }
