...
namespace FlowerShop.Web.Controllers
{
    ...
    public class OrdersController(FlowerDbContext context) : ControllerBase
    {
        ...
        // Для статистики — все заказы без фильтрации статуса
        [HttpGet("stats")]
        public async Task<ActionResult<List<GetOrderDto>>> GetOrdersForStats()
        {
                ...
                .Select(o => new GetOrderDto(
                    ...
                    o.Items.Select(i => new GetOrderItemDto(
                        ...
                        i.Bouquet != null ? new GetBouquetDto(
                            i.Bouquet.Id, i.Bouquet.Name, i.Bouquet.Description,
                            i.Bouquet.Price, i.Bouquet.Quantity, i.Bouquet.ImagePath, i.Bouquet.Rating) : null,
                        i.SoftToy != null ? new GetSoftToyDto(
                            i.SoftToy.Id, i.SoftToy.Name, i.SoftToy.Description,
                            i.SoftToy.Quantity, i.SoftToy.Price, i.SoftToy.ImagePath, i.SoftToy.Rating) : null
                    )).ToList()
                ))
                .ToListAsync();
        ...
        // Для списка заказов в панели — только активные (New/Pending/InProgress)
        [HttpGet]
        ...
        public async Task<ActionResult<List<GetOrderDto>>> SearchOrders([FromQuery] string? name)
        {
            ...
            var orders = await _context.Orders
                ...
                .Where(o => EF.Functions.Like(o.User.Name, $"%{name}%") ||
                           EF.Functions.Like(o.User.Email ?? "", $"%{name}%") ||
        ...
        public async Task<ActionResult<GetOrderDto>> CreateOrders([FromBody] CreateOrderDto dto)
        {
            ...
            try
            {
                ...
                if (user == null)
                    user = await _context.UserDomains.FirstOrDefaultAsync(u => u.Email == login);
                ...
                if (user == null)
                {
                    ...
                    user = new UserDomain
                    {
                        ...
                        Email = string.IsNullOrEmpty(login) ? userName : login,
